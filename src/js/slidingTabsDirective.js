var slidingTabsDirective = angular.module('ionic').directive('ionSlidesTabs', ['$timeout', '$compile', '$interval', '$ionicSlideBoxDelegate', '$ionicScrollDelegate', '$ionicGesture',
    ...
    return {
        ...
        link: function(scope, element, attrs, parent) {
            ...
            var init = function () {
                ...
                if(angular.isDefined( attrs.slideTabsTranslatable ) && attrs.slideTabsTranslatable === "false" ) {
                    options.slideTabsTranslatable = false;
                }
                if(options.slideTabsTranslatable) {
                    tabItems = '<li ng-repeat="(key, value) in tabs track by $index" ng-click="onTabTabbed($event, {{key}})" class="slider-slide-tab"><span ion-slide-tab-translate ng-html-bind="value"></span></li>';
                }else {
                    tabItems = '<li ng-repeat="(key, value) in tabs track by $index" ng-click="onTabTabbed($event, {{key}})" class="slider-slide-tab" ng-bind-html="value"></li>';
                }
                if(options.slideTabsScrollable) {
                    ionicScrollDelegateID = "ion-slide-tabs-handle-" + Math.floor((Math.random() * 10000) + 1);
                    tabsBar = angular.element('<ion-scroll delegate-handle="' + ionicScrollDelegateID + '" class="slidingTabs" direction="x" scrollbar-x="false" overflow-scroll="false" has-bouncing="false"><ul>' + tabItems + '</ul> <div class="tab-indicator-wrapper"><div class="tab-indicator"></div></div> </ion-scroll>');
                } else {
        ...
        controller: ['$scope',function($scope) {
            ...
            this.updateTabsController = function() {
                $timeout(function() {
                    $scope.updatetabsLink();
                });
            }
        }]
    };
...
slidingTabsDirective.directive('ionSlideTabLabel', ['$rootScope', function($rootScope) {
    return {
        require: "^ionSlidesTabs",
        link: function ($scope, $element, $attrs, $parent) {
            $parent.addTab($attrs.ionSlideTabLabel);
            $rootScope.$on('$translateChangeSuccess', function() {
                $parent.updateTabsController();
            });
        }
    }
}]);

slidingTabsDirective.directive('ionSlideTabTranslate', ['$rootScope', '$translate', function($rootScope, $translate) {
    return {
        require: "^?ionSlideTabLabel",
        restrict: 'EA',
        template: '{{translateLink}}',
        controller: ['$scope', function($scope) {
            $scope.translateLink = $translate.instant($scope.value);
            $rootScope.$on('$translateChangeSuccess', function() {
                $scope.translateLink = $translate.instant($scope.value);
            });
        }]
